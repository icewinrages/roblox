...
local function getAllOre()
    ...
    for _, v in ipairs(game:GetService("Workspace").Minerals:GetChildren()) do
        if v:IsA("Part") or v:IsA("MeshPart") or v:IsA("Model") then
            local oreName = v:IsA("Model") and v.Name or v.Name
...
local function drawOreEsp(ore)
    ...
    esp.TextColor3 = ore.model.Color or Color3.new(1, 1, 1)
...
main.Toggle({
    ...
    Callback = function(value)
        if value then 
            ...
            Lighting.Brightness = 2 -- Увеличьте яркость, если необходимо
        else
            Lighting.Ambient = Color3.new(0.5, 0.5, 0.5)
            Lighting.Brightness = 1 -- Вернуть к нормальному
        end
    end
})
